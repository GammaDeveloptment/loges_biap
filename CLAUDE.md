...
>
> **Entrega 0 (Documento 007) — progreso al 2026-08-11:**
> - Monorepo creado en `apps/api` (NestJS), `apps/web` (Next.js — consolida la carpeta `front/`, que queda sin uso), `packages/shared-types`. Repositorio git inicializado, sin commits todavía.
> - Esquema completo de PostgreSQL escrito en `apps/api/prisma/schema.prisma` (Documento 005 + `auditoria_acceso` del Documento 011) — **validado y con cliente Prisma generado**, pero **nunca conectado a un Postgres real**: este entorno de trabajo no tiene acceso a ningún servidor Postgres (ni local/Docker, ni la VPN de Gammacargo). La primera migración real (`npx prisma migrate dev --name init`) queda pendiente de ejecutarse por quien tenga esa conectividad — ver Documento 015, sección 2.
> - Políticas de Row Level Security (Documento 011, sección 5) escritas como referencia en `apps/api/prisma/rls-policies.sql`, con instrucciones de cómo incorporarlas a una migración real — no se pueden aplicar sin una base de datos real.
> - Módulo de autenticación (JWT + bcrypt, matriz de áreas del Documento 011) y CRUD básico de usuarios (Documento 010 §4.6) implementados y compilando sin errores; arranque real del servidor probado hasta el punto esperado (falla explícitamente al no encontrar Postgres — comportamiento correcto, no un bug).
...
> - Pendiente explícito antes de que esta entrega pueda darse por completa: ejecutar la primera migración contra la base de datos dedicada real de Gammacargo (Documento 004 §3) y aplicar `rls-policies.sql`; coordinar acceso VPN de desarrollo, máquina del runner de CI/CD y canal de alertas con el equipo de soporte de Gammacargo (Documento 013).
>
