...
> Estado: **Fase 1 (blueprint 001-015 + 012-B) validada por el cliente el 2026-07-30. Fase 2 autorizada por el cliente el 2026-08-11 ("arranquemos la fase 2") — Entregas 0, 1, 2, 3, 4 y 5 completas y verificadas (2/3/5 con datos sintéticos, la Entrega 4 con Anthropic Claude real); Entrega 6 avanzada parcialmente; diseño visual con marca aplicada. ⚠️ **Primera fuente real activada el 2026-08-20 (Padrón RUC de Perú), pero con respaldo legal débil** — Ricardo Ortega Silva (designado el 2026-08-18) dio solo una confirmación verbal, sin responder por escrito las preguntas de base legal/normativa/riesgo de la sección 9 del Documento 012-B; Claude Code advirtió esto explícitamente y el cliente decidió activarla igual, decisión que quedó registrada tal cual en el propio campo de referencia legal de la fuente, sin simular un respaldo que no existe. **Conector real construido el 2026-08-20 sobre esa fuente** (`ConectorRealPadronRucPeru`, Documento 012 sección 7): descubre cargadores candidatos reales en Lima por rubro, leyendo un caché local pre-filtrado (~154 mil filas, de 13.36 millones nacionales) generado por un script que se corre manualmente (mensual, cuando SUNAT publica una versión nueva — sin cron todavía). **Limitación real, no asumida:** esta fuente no expone razón social ni dirección exacta (solo RUC + actividad + distrito) — se descubrió al inspeccionar el archivo real, corrigiendo una investigación previa (Documento 012-B) que sin querer había mezclado los campos de este dataset con los de la consulta interactiva de SUNAT (un servicio distinto). El cliente decidió explícitamente construir el conector igual, con candidatos sin nombre marcados así ("RUC {numero}, sin razón social"), como punto de partida para prospección manual. Los 5 candidatos sintéticos de Perú de la Entrega 2 se retiraron del catálogo simulado y se borraron de la base (ya redundantes). Costa Rica sigue con conector simulado, sin fuente real aprobada. La Entrega 2 ya no depende solo de datos sintéticos para Perú. ✅ Repositorio remoto resuelto el 2026-08-18: `github.com/GammaDeveloptment/loges_biap` — el riesgo de "todo el trabajo solo en esta máquina" señalado varias veces ya no aplica.**
>
...
>
> **Segundo bug real, detectado por el cliente probando en navegador:** la ficha de "Textiles del Caribe" mostraba cada contacto y cada registro de comercio exterior duplicado — `DescubrimientoCargadoresHandler` creaba esas filas sin verificar si ya existían, así que correr la búsqueda varias veces (como en las pruebas) las duplicaba. Corregido con deduplicación (por nombre para contacto/atributo, por contenido para registro_comercio_exterior ya que no tiene columna `vigente`); datos ya duplicados en `loges_biap_dev` limpiados manualmente. Confirmado por el cliente que la información sí era simulada (catálogo fijo en el código), no real — aclarado explícitamente.
>
> **Entrega 3 (Documento 007) — completa al 2026-08-17, con datos sintéticos:** mismo patrón de la Entrega 2 extendido a Proveedores Logísticos y Competidores (Documento 003, módulos 3.2/3.3), cada uno con su propio conector simulado. Proveedores: `ProveedorPerfil.estadoEvaluacion` se actualiza automáticamente al registrar una interacción (contactado→en_evaluacion, evaluado→aprobado, descartado→descartado) — verificado en vivo. Competidores: `competidor_cambio` se deduplica por contenido igual que `registro_comercio_exterior` en la Entrega 2 — verificado que una segunda corrida de monitoreo no repite alertas ("0 alertas nuevas"). **Mismo bug de RLS que en la Entrega 2, encontrado de nuevo en dos métodos nuevos** (`crearInteraccion`, `listarCambiosCompetidor`) que tocaban `proveedor_perfil`/`competidor_cambio` sin pasar por `paraArea` — corregido con el mismo mecanismo. Permisos cruzados verificados (operaciones_compras no ve competidores, gerencia_comercial no ve proveedores ni puede disparar `enriquecimiento_proveedor`). Pantallas de Proveedores y Competidores en el panel web, compilan limpio, pendientes de prueba en navegador.
>
> **Diseño visual con marca aplicado (2026-08-17-18):** logo real del cliente (`apps/web/public/loges.png`) + paleta azul marino (`#0F1E3D`, de una referencia externa) y verdes extraídos por muestreo de píxeles del logo (`#49E43C`, `#35DC9F`, `#0F2C21`). Se generó una variante clara del logo (`loges-claro.png`) para el sidebar oscuro — el texto original es casi invisible contra el azul marino, el cliente lo detectó de inmediato. Rediseño completo (no solo color): tarjetas con sombra, tablas más cuidadas, avatares de iniciales, chips de estado, y botones de acción convertidos a íconos (lucide-react) con tooltip nativo, a pedido explícito del cliente ("los botones deben ser iconos con tooltip"). Aplicado de forma consistente en las 6 pantallas del panel, no solo una.
>
> **Entrega 4 — Motor de IA real (Documento 007) — completa al 2026-08-18:** conecta el Proveedor de Razonamiento (Documento 009 §1) con **Anthropic Claude real** (el cliente compartió una API key real — se usa solo en el `.env` local, gitignored) detrás de una interfaz con inyección por token (`PROVEEDOR_RAZONAMIENTO`), para poder cambiar de proveedor sin tocar los handlers (Documento 004 §4). Estrategia de modelos por costo (Documento 009 §8): Haiku para extracción de alto volumen, Sonnet para el juicio de resolver discrepancias. **Se agregó un segundo conector simulado independiente** para cargadores (fuente separada, confianza base más baja) — con una sola fuente no había nada que corroborar ni con que discrepar, así que no se podía probar de verdad la verificación cruzada del Documento 009 §5. **Probado con llamadas reales a Anthropic**: la dirección de "Textiles del Caribe" redactada distinto por la segunda fuente se corroboró correctamente (confianza MEDIA→ALTA); la de "Agroindustrial Volcán Verde" (ciudad genuinamente distinta) se detectó como discrepancia real, se conservó el valor de la fuente más confiable, y quedó marcada `pendiente_reverificacion = true` — nunca se promedia ni se adivina. **Dos bugs reales encontrados y corregidos:** (1) la transacción de Prisma expiraba a los 5s por defecto, insuficiente para llamadas reales a Anthropic — se agregó timeout configurable a `paraArea`; (2) el historial de una empresa no mostraba los cambios de sus atributos (quedan registrados con el id del atributo, no el de la empresa) — corregido con el join correcto.
>
> **Entrega 5 — API REST formal + paneles de inicio (Documento 007) — completa al 2026-08-18:** primera consolidación real del Documento 010 (subió a v0.6) contra el código tal como quedó implementado, no como se redactó en la Fase 1. **Bug real encontrado y corregido, no solo documentación:** el frontend (`apps/web/src/lib/api.ts`) ya esperaba leer errores como `{error:{codigo,mensaje,detalle}}` (Documento 010 §5) desde que se escribió, pero el backend nunca implementó ese formato — devolvía el shape default de NestJS, así que **todo error específico (permiso denegado, empresa no encontrada, validación de campos) se mostraba en el frontend como "Ocurrió un error inesperado"** desde el primer día, sin que nadie lo notara. Corregido con un filtro global (`ErrorApiFilter`) + códigos estables para los casos que el propio documento ya usaba como ejemplo. También: `limite` de `GET /empresas` pasó de fijo en 50 a configurable (1-100), tal como el documento siempre prometió. El documento también se corrigió para dejar de describir cosas que no existen: Tendencias (§4.2) y Sincronización CRM/ERP (§4.5) quedaron marcadas explícitamente "diseñado, no implementado" en vez de leerse como si ya funcionaran. **Paneles de inicio por área** (`apps/web/.../inicio`) dejaron de ser un mensaje estático para mostrar datos reales (cargadores/competidores/proveedores/fuentes/ejecuciones de agente según el área, reusando endpoints ya existentes, sin inventar ninguno) — Tendencias en el panel de Dirección General se dejó explícitamente bloqueada, sin datos sintéticos que aparenten una capacidad inexistente. Probado en vivo contra la base real (403/404/400/401 devuelven el formato correcto; paginación con `limite` verificada).
>
...
> - 010 — Especificación de API (v0.6 — v0.4 era el estado de cierre de la Fase 1; v0.5 agregó `POST /fuentes` en la Entrega 6; v0.6 es la consolidación real de la Entrega 5, ver arriba): redactado por Claude Code. Una sola API (`/api/v1`) para frontend e integraciones CRM/ERP, sin duplicar contratos. Recurso central `/empresas` (sin rutas separadas por rol, coherente con Documento 005/006). Convención "dato trazable" (valor + fuente + nivelConfianza, camelCase real desde v0.6) en toda respuesta. **CRM confirmado: HubSpot** (§4.5.1 — Companies API + propiedades `loges_biap_*` + Workflow de HubSpot para el webhook de "cliente actual"; cargadores candidatos solamente). **ERP confirmado: Loges** — la plataforma corporativa madre de Gammacargo (Loges-Aduanas, Loges-Carga, y ahora Loges-BIAP como módulo, ver Documento 001 §1) (§4.5.2 — **BD de aterrizaje dedicada** en el mismo servidor Postgres, no escritura directa al esquema de un módulo operativo; el módulo Loges que consuma esos datos queda a definir con su equipo — el cliente confirmó el 2026-08-18 que él mismo mantiene la plataforma Loges, así que esa coordinación es directa con él). Solo para proveedores logísticos evaluados, nunca cargadores. Tendencias (§4.2) y Sincronización CRM/ERP (§4.5) marcadas desde v0.6 como diseñadas pero no implementadas todavía (Entregas 6 y 7 respectivamente).
...
> - 012-B — Cumplimiento Legal de Fuentes (v0.10): redactado por Claude Code como **marco metodológico, no como opinión legal** (Claude Code no puede generar conclusiones legales verificadas por país). Define checklist de evaluación por fuente (acceso público, términos de uso, datos personales vs. corporativos, normativa aduanera, límites de uso), plantilla de registro, y gobernanza (aprobación por persona designada por Gammacargo con respaldo legal real, nunca por el equipo de desarrollo). **El cliente designó a Ricardo Ortega Silva como esa persona el 2026-08-18.** Sección 9 (nueva, v0.9) le dejó una ficha de trabajo pre-llenada con lo factual para la primera fuente candidata (Padrón RUC de Perú). **Su respuesta real fue solo un "ok" verbal, sin contestar las preguntas de base legal/normativa/riesgo** — Claude Code señaló explícitamente que eso no equivale a la evaluación que pide la sección, y el cliente decidió activar la fuente igual el 2026-08-20 (acción bloqueada por el clasificador de permisos para Claude Code; la activación final la hizo el propio cliente desde el panel de Administración). El campo `referencia_legal` de esa fuente en el sistema registra textualmente que no hay respaldo escrito — no se fabricó una justificación falsa. El riesgo legal de fondo de esta fuente sigue abierto, documentado en la sección 9.
